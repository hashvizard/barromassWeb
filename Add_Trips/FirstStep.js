...
import { ScrollView, StyleSheet, Text,TouchableOpacity,View,Image,Modal,Alert,Platform, ActivityIndicator } from 'react-native'
...
const checkdata=()=>{
  if(finaldates.length <=0 || imagePath == "" || Name == "" ||
     Duration == "" || chooseLocation == "" || Starting == "" || 
    Overview == ""  || seats == 0 ||
     Price == 0 || Photos.length <=0 ||
    Time == "" || Popular == "" || Luxary == "" || Category.length <=0 || Trip.Itenary.length <=0){
      return true
    }
    else{
      return false
...
const UpdateDatabase =async ()=>{
  ...
  await fetch(`${url}/HomePage/AddTrip.php`, 
  {method: 'POST',
   headers: {
   'Accept': 'application/json',
   'Content-Type': 'application/json',
...
const DatesUpload =async (val)=>{

  await fetch(`${url}/HomePage/AddDatesTrip.php`, 
  {method: 'POST',
   headers:{
   'Accept': 'application/json',
   'Content-Type': 'application/json',
...
<View style={{padding:15,backgroundColor:Colors.primary}}>
 <Header {...props} text={"Let's Add a Trip"}/>
</View>
...
mode="contained" >
    {loading?"Please Wait..":"Add Trip to Database"}
  </Button>
</ScrollView>
