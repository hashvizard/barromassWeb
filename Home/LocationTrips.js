...
useFocusEffect(
    React.useCallback(() => {
      if(End && !DataEnd){
        axios.post(`${url}/HomePage/TripsLocation.php`, {
...
const renderItem = ({ item ,index}) => {
  let cat=JSON.parse(item.Category)
    return(<Card
    
    style={{margin:5}} mode="elevated">
        <Card.Actions style={{alignSelf:"flex-end",justifyContent:"space-between",width:"100%"}}>
          <Button color={Colors.Red} >{cat[0]}</Button>
