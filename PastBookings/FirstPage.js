...
import { View,Text,FlatList,TouchableOpacity ,RefreshControl,StatusBar, ActivityIndicator,Alert,Image,ScrollView,StyleSheet} from 'react-native';
import { useFocusEffect } from '@react-navigation/native';
import axios from 'react-native-axios'
import url from '../Constants/Database';
import Colors from '../Constants/Colors';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';
import { Avatar, Button, Card, Title, Paragraph ,Caption,Subheading} from 'react-native-paper';
...
useFocusEffect(
    React.useCallback(() => {
      if(End && !DataEnd){
        axios.post(`${url}/HomePage/PastBookings.php`, {
         offset:offset
        })
...
const renderItem = ({ item ,index}) => {
    ...
    return(<Card style={{margin:5}} mode="elevated">
      <TouchableOpacity
         onPress={()=>props.navigation.navigate("TripDetails",{id:item.Id,imag:item.Image,Bookid:item.bookId})}
         >
...
<Text>{item.BookingAmount == 0?"Full Payment":"Remaing Amount"} : ₹ {item.FullAmount}</Text>
    ...
    return (
      ...
      <StatusBar backgroundColor={Colors.ORANGE} />
         ...
         <Title style={{marginLeft:15,color:"white"}}>Past Booking's</Title>
         </View>    
